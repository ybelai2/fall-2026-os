...
interface EventModalProps {
  ...
  defaultDate: string;
  existingEvent?: CalendarEvent | null; // <-- NEW: Allows us to pass in an event to edit
}

const CATEGORIES: Category[] = ['CLASS', 'CAREER', 'CS / TECHNICAL', 'STUDY', 'PROJECT', 'FAITH', 'BODY', 'PERSONAL'];

export function EventModal({ onClose, onSave, defaultDate, existingEvent }: EventModalProps) {
  // If we are editing, pre-fill with the existing data. Otherwise, use defaults.
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Quick hack to safely get the day of the week (0 = Sun, 1 = Mon) without timezone bugs
